...
import 'package:chat_app/src/Views/Chat/right_message.dart';
...
class ChatScreenState extends State<ChatScreen> {
  ...
  List<ChatMessage> listMessage = new List.from([]);
  int _limit = 20;
  ...
  // final _getChatHistory = GetChatHistoryBloc();
  ...
  Future uploadFile() async {
    try {
      imageUrl = await _database.uploadImage(imageFile, 'ChatImage/$id/');
      setState(() {
        isLoading = false;
        onSendMessage(imageUrl, MessageType.IMAGE);
      });
    } catch (e) {
      print('error during upload image in chat $e');
      setState(() {
        isLoading = false;
      });
          ...
          Flexible(
            child: Container(
              child: TextField(
                ...
                style: TextStyle(color: Variables.appColor, fontSize: 15.0),
      ...
      child: groupChatId == ''
          ? Center(
              child: CircularProgressIndicator(
                  valueColor:
                      AlwaysStoppedAnimation<Color>(Variables.appColor)))
          : GestureDetector(
              onTap: () {
              ...
              child: Container(
                child: StreamBuilder<QuerySnapshot>(
                  stream: FirebaseFirestore.instance
                      ...
                      .limit(_limit)
                      .snapshots(),
                  // _getChatHistory.chatHistoryStream,
                  builder: (context, snapshot) {
                    if (!snapshot.hasData) {
                      ...
                      ChatModel chatModel =
                          ChatModel.fromJson(snapshot.data.docs);
                      listMessage.addAll(chatModel.messages);
                      return ListView.builder(
                        padding: EdgeInsets.all(10.0),
                        itemBuilder: (context, index) =>
                            buildItem(index, chatModel.messages[index]),
  ...
  Widget buildItem(int index, ChatMessage chatMessage) {
    if (chatMessage.idFrom == id) {
      // Right (my message)
      return RightMessage(
